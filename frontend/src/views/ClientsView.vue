...
const columns = computed<GridColumn[]>(() => [
  ...
  { key: 'email',   label: 'Email',     type: 'text', editable: isAdmin.value, width: '200px' },
  { key: 'phone',   label: 'Téléphone', type: 'text', editable: isAdmin.value, width: '140px' },
...
async function onUpdate(id: number, field: string, value: any) {
  await store.updateClient(id, { [field]: value } as any);
}
...
async function onDuplicate(ids: number[]) {
  const originals = store.clients.filter(c => ids.includes(c.id));
  await Promise.all(
    originals.map(c =>
      store.createClient({
        name: `${c.name} (copie)`,
...
function openEdit(c: Client) {
  ...
  form.value = { name: c.name, email: c.email, phone: c.phone, address: c.address, notes: c.notes };
  formError.value = '';
  showForm.value = true;
}
...
async function submitForm() {
  ...
  try {
    if (editingId.value) {
      await store.updateClient(editingId.value, form.value);
    } else {
  ...
  <div class="page">
    ...
    >
      ...
      <template v-if="isAdmin" #row-actions="{ row }">
        <button class="act-btn edit" @click="openEdit(row as Client)" title="Modifier">✏</button>
      </template>
    </DataGrid>
    ...
    <div v-if="showForm" class="overlay" @click.self="showForm = false">
      <div class="modal">
        ...
        <form @submit.prevent="submitForm">
          <label>Nom *</label>
          <input v-model="form.name" required placeholder="Nom du client" />
          <div class="row2">
            <div>
              <label>Email</label>
              <input v-model="form.email" type="email" placeholder="contact@client.com" />
            </div>
            <div>
...
.modal { background: white; border-radius: 12px; padding: 1.75rem; width: 100%; max-width: 480px; box-shadow: 0 8px 32px rgba(0,0,0,.15); }
